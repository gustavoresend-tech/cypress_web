...
describe('Verifica Etapa "Assinatura"', () => {
    beforeEach(() => {
    ...
    it('Verifica Etapa "Assinatura"', () => {
        // Acessa o primeiro card dispoível no worlflow
        cy.contains('Teste-QA-Funcionais - (NAO USAR)').click();

        // Verifica se a atividade "Assinturas no cartório" está disponível e clica
        cy.get('[style="max-width: calc(100vw - 320px);"]').should('be.visible')
        .contains('Assinaturas no cartório').click();
        cy.get('#btn-iniciar-atividade').click();
        // Verifica campos

        // Captura print para evidencia
        cy.screenshot('23-Assinaturas no cartório',{overwrite: true});
